...
### 11. Yoga/Dosha Engine (යෝග සහ දෝෂ) — Separated Tags with Structured Evaluations

- The horoscope detail page renders **Yoga tags and Dosha tags as two separate groups** — never a combined list. Each present yoga/dosha expands into a detail panel: bilingual name, "why" (every satisfied formation rule with the exact planetary conditions), "what result" (house-specific themes, tendency language only), and cancellation status with tradition provenance — mitigation shown as a distinct subsection; cancelled tags render with a strikethrough (Wargoththama-tag pattern). **Read-only on all views** (own/public/share-link) — no editing, no overrides (US-YD-001..US-YD-007; `specs/business-analysis/20260906-0707-yoga-dosha-tags.md`).
- The evaluation engine is a **new pure shared module `src/lib/yogaDosha/`** (`types.ts`, `catalog.ts`, `relationships.ts`, `rules.ts`, `ruleEngine.ts`, `interpretation.ts`, `cancellation.ts`, `index.ts`) — no ephemeris, no I/O, same contract as `shadBalaya.ts`/`wargaKendara.ts`: formation rules evaluate over the already-stored `planets`/`houses`/`ascendant` (reusing the stored per-planet `aspects[]`, including the manual-chart derived aspects), never Chart documents.
- **Numeric enums** `YogaStrength` (1 = Very Strong … 4 = Weak) and `CancellationStatus` (1 = Not Cancelled, 2 = Cancelled, 3 = Mitigated) are added to `src/lib/astrologyEnums.ts` per the BA data-model tables; all display strings resolve via i18n (`yoga.*`, `dosha.*`, `yogaStrength.*`, `cancellationStatus.*` keys) per locale — nothing localized is stored.
- **Structured output** follows the BA data-model JSON shape for both yogas and doshas: `formation` (all triggered rule IDs, ranked `primaryRule`, numeric strength, per-rule reasons with numeric enum params), `context` (houseImpact + involved planets), `interpretation` (house-specific theme keys), `mitigation` (factors with ruleId/factor/effect/tradition/confidence), `cancellation` (status + factors — **only a registered cancellation rule may set status 2**; mitigation alone may set status 3 at final assessment), `finalAssessment` (weighted severity + expression keys), `dashaActivation` (soft dasha note, never a timing prediction). Doshas use the identical shape plus `isPresent` (preserved for the search result cards, route and export). `CalculationResult.yogas` is retyped `unknown[]` → `YogaEvaluation[]`; `DoshaInfo.doshas` → `DoshaEvaluation[]`.
- **Static catalog** (`YOGA_DOSHA_CATALOG` in `src/lib/yogaDosha/catalog.ts`) is the only place the calculation path references yoga/dosha identity — engine, storage, UI and search are generic and catalog-driven. New entries are added **incrementally one-by-one** (US-YD-013): catalog row + deterministic rule functions + i18n keys + search-vocabulary entries + tests, additive and non-breaking. Current dosha entries: `manglik` (`dosha`, `pending-domain` — skipped by the engine until the formation + cancellation rules are domain-confirmed); `shaniMangala` — reclassified to the **dosha group** (rev 3) as **Shani Mangala Dosha**, now narrowed (rev 6/7/9) to 3 rules (SM-01 conjunction — same sign + house AND stored mutual 0° records, within the per-planet orbs (corrected 6a74b291: 13.57° apart → not combined); SM-02 7th-from-each-other + mutual 180° drishti record; SM-03 mutual 4-10 + mutual 90°/270° drishti record — both directions must be stored, i.e. within the per-planet orbs; corrected against real chart `6a68e773…`); and `agniMarutha` (`dosha`, active, rev 6) with 8 broad Saturn–Mars rules (am01 conjunct, am02/03 aspect, am04/05 sign ownership, am06/07 nakshatra lord, am08 parivartana) per `docs/agni-marutha-dosha.md` — Shani Mangala is therefore a mutual-drishti subset of the broader Agni Marutha; `yogas` list empty this phase; `yogaDoshaVersion` is `4` (pre-v4 documents recompute from chart facts at render/search via the resolver).
- **Evaluation timing** — calc-time persistence for **both** sources: auto inside `calculateHoroscope`, manual inside `synthesizeCalculation`, persisted automatically via the existing `...calculated`/`...synth` spread (`ICalculatedDetails extends CalculationResult`). The AstrologySettings full recalculation job needs **no code changes** (no user overrides exist). Legacy documents fall back to a render-time pure recompute via `resolveYogas`/`resolveDoshas`; a stored **`yogaDoshaVersion`** marker on `CalculatedDetails` distinguishes "computed, possibly empty" from "legacy" (the previously-stored `[]` alone is ambiguous).
- **Search (US-YD-011)**: `calc.yogas`/`calc.doshas` reads in `src/lib/search/textContent.ts` become catalog-name-based (`yoga.<id>.name`/`dosha.<id>.name` resolved per locale, with the legacy string `name` fallback kept); yoga/dosha names join the search vocabulary (`src/lib/search/vocabulary.ts`) so specific-name queries work in Sinhala + English; `/api/search` gains generic `yoga`/`dosha` catalog-id exact conditions.
- **No new API routes** — the existing `GET /api/horoscope/:id` and `GET /api/share/:token` payloads grow additively. No MongoDB migration (storage stays `yogas: [Schema.Types.Mixed]` / `doshas: Schema.Types.Mixed`).
- Full spec: `specs/architecture/20260906-0905-yoga-dosha-tags-architecture.md`
