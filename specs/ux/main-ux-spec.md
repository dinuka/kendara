...
**Last Updated:** 2026-09-06 (rev: Yoga/Dosha Tags (යෝග සහ දෝෂ) — the horoscope detail page gains a new 5th tab "Yogas & Doshas" (යෝග සහ දෝෂ, `horoscope.yogaDoshas` label key) between the Dashas tab and the Metadata tab, hosting a dedicated read-only section with two visually separated and independently empty-stated tag groups — Yoga (indigo accent) and Dosha (amber accent) — each tag showing the catalog i18n name, a `yogaStrength.*` severity pill (final-assessment), and a `cancellationStatus.*` state treatment (struck-through name + badge when cancelled, "Reduced" badge when mitigated, dashed-gray "Not available" non-interactive tag for unregistered legacy catalog IDs; dot + tint + aria-label so color is never the only channel); each present tag expands an in-place multi-open disclosure panel composed purely from the stored structured evaluation (yoga name + tradition provenance, WHY it forms — primary-rule headline sentence + every satisfied-rule reason line with numeric-enum params resolved per locale, WHAT result — house-grouped tendency themes, explicit cancellation status line or per-factor cancellation rules with tradition labels, a visually distinct mitigation subsection, optional soft dasha-activation note); read-only on own/public/share views, no new endpoints, no loading (structured data ships with the page; legacy docs recompute at render via `resolveYogas`/`resolveDoshas`); search result cards keep their existing combined sections this phase (catalog-name resolution + legacy fallback only); `yogaDosha.*`/`yoga.*`/`dosha.*`/`yogaStrength.*`/`cancellationStatus.*`/`tradition.*` key set added to both locales (domain strings SI-pending → EN fallback; Sinhala translations completed 2026-09-06); see `specs/ux/20260906-0945-yoga-dosha-tags.md`. Rev 3 (2026-09-06, engineer direction): Shani Mangala reclassified to the Dosha amber group as **Shani Mangala Dosha / ශනි මංගල දෝෂය** (render + search resolve via the `dosha.*` namespace; the Yoga group is empty this phase, `yoga` namespace emptied — tag/panel names, panel ids `dosha-shaniMangala-panel`, severity pills and cancellation treatments are unchanged). Rev 5 (2026-09-06, user direction): dosha tag chips are color-coded by state — **present dosha = red chip** (`bg-red-50` text-red-700, red dot), **cancelled dosha = gray chip** (gray-100, line-through, gray dot); yoga chips keep the neutral white/gray treatment with the indigo hover. Rev 6 (2026-09-06, user direction per `docs/agni-marutha-dosha.md`): Shani Mangala narrowed to 3 positional-only rules (sm01 conjunction / sm02 7th / sm03 4-10) and a second active dosha added — **Agni Marutha Dosha / අග්නි මාරුත දෝෂය**, 8 broad Saturn–Mars rules (am01..am08 incl. parivartana and nakshatra-lord clauses) with `dosha.agniMarutha.*` house1..12 themes — the red/gray chip and panel-block treatment apply identically (catalog-driven; no component changes). Previous rev: Student Notepad (ශිෂ්ය සටහන් පොත) — the horoscope detail page gains a draggable/resizable `z-40` notepad popup (default right-docked 480×640, bounds 320×400–900×1200) opened from a header-action button with a saved-content dot; every viewer gets a per-horoscope private notepad; the parent-tag strip (12 houses + Other, wrap, `aria-pressed` radio semantics) drills into sub-tag chips and drives seven fixed-order system observation sections (planets in house, house load, nakshatra load, significator planets, Chandra/Surya Lagna related house, related Warga Kendara) with `(n/m)` green-ratio badges, green/red/white classification chips (dot + tint + aria-label — color is never the only channel), plus student observation tags (six-color TagColor palette, dashed-square chips, default White) grouped by context and newest-first result notes (caps 500/2000, 200/100 counts); 500ms-debounced auto-save with optimistic UI + rollback toast + keepalive flush on close/navigate, full restore of geometry/open-state/selection on reopen; `notepad.*` key set added to both locales (catalog strings SI-pending → EN fallback); see `specs/ux/20260816-1803-student-notes.md`. Previous rev: Warga Kendara (වර්ග කේනදර) — the horoscope detail Charts tab replaces the fixed "Birth & Navamsa" chart pair with a varga tab strip: Rāśi (D1) is permanently pinned as the first chart (lock glyph, always selected, click = no-op), the second chart is selectable among Navāṁśa (D9, default), Surya Lagna and Chandra Lagna (exactly two charts shown), "Other Charts" is reduced to House only, each displayed chart gains its own Houses + Planets tables directly beneath the figure (Nakshatra (Pada) and Bhava Suchika columns D1-only; Conjunctions/Aspects render without degree differences; the D1 `Other` cell carries the 12 D1-only flags, every chart's `Other` carries Maraka/Maranakaraka/Dig Bala) plus each chart's main indication as tag chips beneath the caption — display-only on all views, both chart sources, render-time legacy fallback, no user overrides; see `specs/ux/20260815-1203-warga-kendara.md`. Previous rev: Bhava Suchika (භාව සුචික) — the Calculations tab Lagna section gains a "භාව සුචික" house-index tag (label + `7 — සප්තමාංශකය`) alongside the Wargoththama/Gandamula flags, and the planets table gains a new "භාව සුචික" column showing each planet's Navamsa-derived house index (1–12) with its named form — display-only, both chart sources (manual only when Navamsa data has been entered), render-time legacy fallback, bilingual explanation tooltip; see `specs/ux/20260814-2130-bhava-suchika.md`. Previous rev: Shad Bala (ෂඩ් බලය) — the horoscope detail Calculations tab gains a 6-strength × 9-planet checkbox table (Sthana / Cheshta / Kala / Dig / Drishti / Naisargika Bala + ratio `(n/6)`) with per-cell reason tooltips for checked and unchecked states, 500ms-debounced auto-save overrides marked by an indigo dot that survives recalculation, and a read-only mode for non-owners and share links; see `specs/ux/20260813-2030-shadbalaya.md`. Previous rev: System-Wide Astrology Settings — the three settings (Orbs / Planet Aspects / Rashi Aspects) are now system-wide on the same `/settings` page: super-admin edits + sees version metadata, recalculation progress, and audit/history; students view read-only with a "managed by the administrator" notice; see `specs/ux/20260813-1000-system-astrology-settings.md`)
**Author:** UX (BMAD)
...
> **Warga Kendara UX:** The Warga Kendara (වර්ග කේනදර) varga-chart viewer has a dedicated UX spec at `specs/ux/20260815-1203-warga-kendara.md` covering the Charts-tab varga tab strip (D1 permanently pinned with lock glyph, second chart selectable among Navāṁśa D9 default / Surya Lagna / Chandra Lagna, click-on-selected = no-op, mobile horizontally scrollable), the per-chart Houses + Planets tables rendered beneath each displayed figure (D1-only Nakshatra (Pada) and Bhava Suchika columns, Conjunctions/Aspects without degree differences, D1 `Other` cell with the 12 D1-only flags, per-chart `Other` with Maraka/Maranakaraka/Dig Bala), each chart's **main indication shown as tag chips beneath the caption** (no separate reference table), the D9 no-data placeholder for manual horoscopes without Navamsa data, the `aria-pressed` toggle-button semantics (two simultaneous selections, so tablist semantics do not apply), and the full `astrology.wargaKendara.*` EN/SI key set.

> **Student Notepad UX:** The Student Notepad (ශිෂ්ය සටහන් පොත) has a dedicated UX spec at `specs/ux/20260816-1803-student-notes.md` covering the header-entry button with saved-content dot, the draggable/resizable non-modal popup (`role="dialog" aria-modal="false"`, z-40 below modals/toasts, default right-docked 480×640, bounds 320×400–900×1200, viewport clamp wins over min, `Ctrl+Alt+Arrow` nudge / `Ctrl+Alt+Shift+Arrow` resize, auto-open on restore), the wrap parent-tag strip (12 houses + Other, `aria-pressed` radio semantics, roving tabindex, parent click resets sub-tag, Other hides sub-tags + observations), the seven fixed-order system observation sections with the single ratio rule (badge renders when section has ≥ 1 tag — `(0/m)` all-white, `(0/1)` not-available; zero-tag sections show empty text and no badge), green/red/white classification chips with dot + tint + aria-label (color never the only channel), student tags as dashed-square TagColor chips (palette green/red/white/blue/yellow/purple = 1–6, default White, 6-swatch radiogroup picker) grouped by context with overview grouping before any selection, newest-first result notes, caps 500/2000 + 200/100 count caps, per-horoscope instant swap, 500ms-debounced auto-save with optimistic UI + rollback toast + keepalive flush, full restore of geometry/open-state/selection on reopen, and the `notepad.*` EN/SI key set (catalog strings SI-pending → EN fallback).

> **Yoga/Dosha Tags UX:** The Yoga/Dosha separated tags feature has a dedicated UX spec at `specs/ux/20260906-0945-yoga-dosha-tags.md` covering the Calculations-tab "Yogas & Doshas" section (between the Lagna summary card and the Houses section), the two visually separated tag groups (Yoga indigo accent vs Dosha amber accent, independent empty states), the tag states (present / cancelled strikethrough+badge / mitigated "Reduced" badge / legacy unknown "Not available"), the multi-open in-place disclosure panels (why-it-forms reason lines + house-grouped result themes + explicit cancellation + distinct mitigation subsection + soft dasha note, all composed render-time from stored numeric enums and i18n keys), `aria-expanded`/`role="region"` semantics, Sinhala width allowances, the full `yogaDosha.*`/`yoga.*`/`dosha.*`/`yogaStrength.*`/`cancellationStatus.*`/`tradition.*` EN/SI key set (seed dosha keys `dosha.shaniMangala.*` sm01..sm03 reasons — sm02/sm03 require the mutual graha drishti per Rev 7 and sm01 the mutual 0° conjunction records per Rev 9 — and `dosha.agniMarutha.*` am01..am08 broad Saturn–Mars rules per `docs/agni-marutha-dosha.md` — Rev 6, plus the Manglik pending-domain keys), and the explicit decision that search result cards keep their existing combined sections this phase.
...
### Warga Kendara Tab Strip (වර්ග කේනදර)

> Full spec: `specs/ux/20260815-1203-warga-kendara.md`

- **Placement**: Horoscope detail Charts tab — replaces the fixed "Birth & Navamsa" chart pair (currently `page.tsx:1084-1106`) with a varga tab strip; the existing Charts/Calculations/Dashas section tabs above it are unchanged
- **Strip semantics**: exactly two charts shown at once — Rāśi (D1) is **permanently pinned** as the first chart (lock glyph, `aria-hidden`, always visually selected; clicking it is a no-op), the **second chart** is selected among Navāṁśa (D9, default), Surya Lagna, Chandra Lagna; clicking the currently-selected second chart is a no-op; clicking any other tab swaps the second chart in place (150ms crossfade)
- **Implementation semantics (a11y)**: the strip is NOT a `role="tablist"` — tablist semantics allow exactly one selected tab, but this UI has two simultaneous selections (D1 + second chart). Use a labeled `role="group"` of toggle buttons with `aria-pressed` (matching the existing Other Charts buttons pattern); D1 renders as a pinned non-interactive badge (`<span>`, `tabindex="-1"`); roving tabindex within the group, arrow keys move between buttons, Space/Enter toggles; group `aria-label` = "Select second chart" / "දෙවන සටහන තෝරන්න"
- **Tab labels**: `රාශි (D1)` / `Rāśi (D1)`, `නවාංශක (D9)` / `Navāṁśa (D9)`, `සූර්ය ලග්න` / `Surya Lagna`, `චන්ද්ර ලග්න` / `Chandra Lagna`; pill styling reuses the existing active-tab treatment (`bg-indigo-600 text-white` selected, `text-gray-600 border` unselected, 40px touch targets)
- **Other Charts**: reduced to House only (the warga entries move into the strip); House button keeps its existing `aria-pressed` behavior
- **Indication tags**: each displayed chart's main indication renders as tag chips directly beneath the caption (inside the same figure card, above the Houses table), comma-split from the i18n `astrology.wargaKendara.vargas.*.{name|indication}` catalog string (e.g. D1: සමස්ත ජීවිතය · ශරීරය · පොදු තත්ත්ව); no separate 16-varga reference table (product decision 2026-08-15); D2–D60 are omitted entirely in phase 1
- **Per-chart tables**: each displayed figure gains its own card section beneath the SVG — Houses table (`#`, `Sign`, `Planets`, `Aspects`) then Planets table (`Planet`, `Sign`, `Str`, `House`, `Conjunctions`, `Aspects`, `Other`); Nakshatra (Pada) and Bhava Suchika columns render **only on the D1 Planets table** (omitted entirely, never empty, on other charts); Conjunctions/Aspects render planet names/chips **without degree differences**; D1 `Other` cell shows the 12 D1-only flags (Wargoththama, Pushkara, Gandantha, Gandamula, 64th Navamsa Lord, 22nd Drekkana Lord, Cheshta Bala, Ashtamansha, Kala Bala, Atmakaraka, Combust, Badhaka) as compact chips; every chart's `Other` cell also carries Maraka / Maranakaraka / Dig Bala; empty cell = `—` (existing convention)
- **No-data states**: manual horoscopes without entered Navamsa data → the second slot shows the existing `astrology.noChartData` placeholder ("සටහන් දත්ත නොමැත"); auto charts always populated; legacy documents resolve via render-time pure-function fallback (first paint populated, no flicker)
- **Read-only everywhere**: own view, public view, and share links render identically; no editing affordances, no mutation endpoints
- **i18n**: `astrology.wargaKendara.*` key set added to both `en.json` and `si.json` (title, tabs.*, pinned aria, secondChartAria, indicationLabel, vargas.d1..d60 + suryaLagna/chandraLagna name/indication); table captions reuse `astrology.houses` / `astrology.planets`; column headers and flag labels reuse existing `astrology.*` keys; no hardcoded strings

### Student Notepad (ශිෂ්ය සටහන් පොත)

> Full spec: `specs/ux/20260816-1803-student-notes.md`

- **Entry point**: header-action button (first position in the `page.tsx:1019` action row, available to all viewers) — icon + short label `සටහන් පොත`/`Notepad` (icon-only < 640px), indigo active fill while open, small indigo content dot when saved tags/notes exist
- **Popup shell**: non-modal `role="dialog" aria-modal="false"`, `z-40` (below modals/toasts/tooltips at z-50); default right-docked 480×640 (`x = vw − w − 16`, `y = 80`); min 320×400 / max 900×1200, clamped to viewport (viewport wins over min on small screens); drag via header (pointer events, `touch-action: none`, 44px min height), resize via bottom-right corner handle, geometry committed on pointer-up; close = header `✕` / `Escape` / button toggle — no outside-click dismissal; keyboard geometry: `Ctrl+Alt+Arrow` nudge 8px, `Ctrl+Alt+Shift+Arrow` resize 16px (plain `Alt+Arrow` avoided — browser back/forward), "Reset position" button; open/close 150ms fade + 8px translate
- **Parent-tag strip**: 13 wrap chips (12 houses + Other `0`), format `<number> — <purpose>`, `role="group"` of `aria-pressed` toggle buttons with roving tabindex + arrow keys (single-select radio semantics; click on selected = no-op); selecting a parent resets the sub-tag selection; "Other" renders no sub-tag row and no observation sections; no selection on first open → localized hint + overview student-tag grouping; sub-tag chips: second labeled group, `text-xs`, same semantics
- **Observation sections** (fixed order): planets in house → house load & position → nakshatra load & position → significator planets → Chandra Lagna related house → Surya Lagna related house → related Warga Kendara; scroll (no accordion), each header carries a right-aligned `(n/m)` green-ratio badge; **ratio rule**: badge renders when the section has ≥ 1 tag (`(0/m)` all-white, `(0/1)` not-available); zero-tag sections show localized empty text with no badge; `calculatedDetails` null → all sections not-available; planet chips inline = glyph + name · sign · house, tooltip shows strength + nakshatra (pada) + house (conjunctions/aspects deferred — data contract question)
- **System classification chips** (green/red/white, render-time only, never stored): rounded-full pills with leading `●` dot + tinted border/bg (`aria-label` includes classification word — color is never the only channel); **student tag chips** (TagColor 1–6 green/red/white/blue/yellow/purple): dashed 2px square chips, tinted, hover edit pencil + delete `×`; 6-swatch radiogroup picker with ✓ + ring, default = White (neutral); student tags grouped by context (overview grouping before any selection)
- **Result notes**: newest-first cards below tags; composer textarea maxLength 2000; caps 500 (tags) / 2000 (notes) via `maxLength` + live counter (amber ≥80%, red at 100%); count caps 200 tags / 100 notes disable composers with localized message
- **Persistence**: 500ms debounce, last-write-wins, optimistic UI first, PUT full doc, silent success, failure → rollback + `notepad.toast.saveFailed` (3s auto-clear); keepalive flush on close/navigate (old id) + beforeunload; per-horoscope isolation — switching horoscopes flushes pending then applies the target horoscope's own geometry/isOpen/selection; restore-on-reopen auto-opens when saved `isOpen === true`; stale `subTag` falls back to parent-only
- **i18n**: new top-level `notepad.*` block in both `en.json` and `si.json` (title, button, close, hasContent, resetPosition, dragHandle, resizeHandle, toast.saveFailed, hint, housePurpose.0..12 {label|sanskrit}, sub-tag + signification word keys, observation.* section headers/labels/empty/notAvailable/classification/ratio, tagColors.1..6, studentTags.*, resultNotes.*, aria.*); catalog strings (purposes, sub-tags, Sanskrit names, significations, section headers) SI-pending → EN fallback; planet/sign/nakshatra display names reuse existing `astrology.*` keys

### Yoga & Dosha Tag Groups (යෝග සහ දෝෂ)

> Full spec: `specs/ux/20260906-0945-yoga-dosha-tags.md`

- **Placement**: Horoscope detail — a new 5th tab "යෝග සහ දෝෂ / Yogas & Doshas" (`id: "yoga-doshas"`, label key `horoscope.yogaDoshas`, inserted between `dashas` and `metadata` in the tab strip, owned by `{activeTab === "yoga-doshas" && calculatedDetails && …}`) hosting a dedicated `bg-white rounded-lg border p-4` section (`id="yogas-doshas"`); no new API, no loading (structured data renders from the stored evaluation; legacy docs recompute pure at render via `resolveYogas`/`resolveDoshas`)
- **Two separated groups**: Yoga group (indigo accent, `text-indigo-700 bg-indigo-50 border-indigo-200` — the Wargoththama/Bhava Suchika pill treatment) and Dosha group (amber accent, `text-amber-700 bg-amber-50 border-amber-200` — caution, never error-red) — never a combined list; each group has its own label chip (`යෝග`/`Yogas`, `දෝෂ`/`Doshas`), a `(n)` count badge (only when ≥ 1 tag, notepad ratio-badge precedent), and its own empty state (`යෝග නොමැත`/`No yogas`, `දෝෂ නොමැත`/`No doshas`); legacy never-evaluated and computed-empty share one neutral empty state
- **Tag anatomy**: clickable chip = leading status dot (`aria-hidden`) + catalog name (i18n-resolved) + `yogaStrength.*` severity pill (final-assessment) + optional state badge; present = indigo/amber dot, no badge; **cancelled** = gray dot + struck-through name (`line-through`, Wargoththama precedent) + `Cancelled` badge; **mitigated** = amber dot + normal name + `Reduced` badge; **unregistered legacy catalog ID** = dashed gray non-interactive chip with raw id + `Not available` badge (warn log, never a crash); color is never the only channel — the state word ships in the badge text and in the tag `aria-label` (notepad chip convention)
- **Detail panel**: in-place multi-open disclosure (open one or many; click the open tag's button to close) — tag is `<button aria-expanded aria-controls>` and the panel is `role="region" aria-labelledby`; panel blocks in fixed order: header (name + `Tradition: {tradition}` provenance line + strength pill + state badge) → **Why it forms** (primary-rule headline sentence composed from `formation.primaryRule`'s reason object, then one line per `formation.reasons[]` — every satisfied rule, deduped, numeric params resolved via existing planet/sign/house i18n maps) → **What result it gives** (themes grouped per house by the `house{n}` segment of the theme keys, tendency wording only, block omitted when `interpretation.themes` is empty) → **How it expresses** (`finalAssessment.expressionKeys`, tendency lines) → **Cancellation** (`cancellation.status` line — explicit "Not cancelled" + hint, `Cancelled by {rule}` + per-factor condition/tradition lines, or the "reduced by mitigating factors" pointer) → **Mitigation** (distinct subsection, one line per `mitigation[]` factor: factor + effect + target + tradition + confidence) → **Dasha activation** (soft note from `dashaActivation.noteKey` + planets, only when present)
- **i18n**: `yogaDosha.*` shared chrome (section title/tooltip, group aria labels, tag aria template, badges, panel headings, cancellation lines, house-group heading), `yoga.*`/`dosha.*` per-group keys + seed-content keys (`dosha.shaniMangala.*` sm01..sm03 rules — sm02/sm03 need the mutual drishti (Rev 7), sm01 the stored mutual 0° conjunction records (Rev 9) — house theme keys, expression, mitigation factors, dasha note; `dosha.agniMarutha.*` am01..am08 broad Saturn–Mars rules with house1..12 themes — Rev 6; `dosha.manglik.*` pending-domain), `yogaStrength.1..4` and `cancellationStatus.1..3` enum labels, `tradition.MAIN_STREAM` provenance label — all in both `en.json` and `si.json`, domain-content Sinhala SI-pending → EN fallback (Sinhala translations completed 2026-09-06); no hardcoded strings, no raw numerics
...
| `specs/ux/20260815-1203-warga-kendara.md` | Warga Kendara (වර්ග කේනදර) varga-chart viewer UX specification (pinned-D1 + selectable second chart strip, main-indication tags per chart, per-chart Houses/Planets tables, D9 no-data placeholder, read-only everywhere) |
| `specs/ux/20260816-1803-student-notes.md` | Student Notepad (ශිෂ්ය සටහන් පොත) UX specification (draggable/resizable z-40 popup, 13-topic parent strip + sub-tags, seven observation sections with `(n/m)` ratios and green/red/white classification chips, TagColor student tags + picker, result notes, caps, debounced auto-save with rollback + keepalive, restore-on-reopen, per-horoscope isolation, `notepad.*` EN/SI keys) |
| `specs/ux/20260906-0945-yoga-dosha-tags.md` | Yoga/Dosha separated tags UX specification (Calculations-tab "Yogas & Doshas" section with two independently empty-stated tag groups — Yoga indigo / Dosha amber, tag states incl. cancelled strikethrough+badge and mitigated "Reduced", unregistered-ID "Not available" fallback — multi-open disclosure panels composed from the stored structured evaluation with why/result/cancellation/mitigation/dasha blocks, numeric params resolved per locale, `yogaDosha.*`/`yoga.*`/`dosha.*`/`yogaStrength.*`/`cancellationStatus.*`/`tradition.*` EN/SI key set incl. Shani–Mangala seed keys, search-card compatibility decision, read-only everywhere) |
