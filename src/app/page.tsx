          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left px-4 py-2 font-medium text-gray-600">{t("horoscope.birthDate")}</th>
                <th className="text-left px-4 py-2 font-medium text-gray-600">{t("horoscope.location")}</th>
                <th className="text-center px-4 py-2 font-medium text-gray-600">{t("horoscope.public")}</th>
              </tr>
            </thead>
            <tbody>
              {horoscopes.map((h) => (
                <tr key={h._id} className="border-t hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/horoscopes/${h._id}`)}>
                  <td className="px-4 py-3 font-medium text-indigo-600">{h.name}</td>
                  <td className="px-4 py-3 text-gray-500">
                    {new Date(h.birthDate).toLocaleDateString(locale === "si" ? "si-LK" : "en-US")} {h.birthTime}
                  </td>
                  <td className="px-4 py-3 text-gray-500">{h.location || (h.latitude ? `${h.latitude}, ${h.longitude}` : "")}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${h.isPublic ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
