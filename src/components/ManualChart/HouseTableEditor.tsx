...
interface HouseTableEditorProps {
    lagna: number;
    houses: Record<number, number[]>;
    computedHouseByPlanet?: Record<number, number>;
    onAdd: (house: number, planet: number) => void;
    onRemove: (house: number, planet: number) => void;
}

export default function HouseTableEditor({
    lagna,
    houses,
    computedHouseByPlanet,
    onAdd,
    onRemove,
}: HouseTableEditorProps) {
    const { t } = useI18n();
    const MAX_PLANETS_PER_HOUSE = 8;
    const houseSigns = deriveHouseSigns(lagna);
    const placed = new Set<number>();
    Object.values(houses).forEach((planets) => planets.forEach((p) => placed.add(p)));
                ...
                <table className="w-full table-fixed text-sm">
                    ...
                    <tbody>
                        {houseSigns.map((sign, i) => {
                            ...
                            const rowPlanets = houses[houseNumber] ?? [];
                            return (
                                    ...
                                    <td className="py-2 pr-3">
                                        <div className="flex flex-wrap gap-1">
                                            {rowPlanets.map((planet) => {
                                                const computedHouse = computedHouseByPlanet?.[planet];
                                                const differs =
