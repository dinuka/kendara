...
import { DoshaEvaluation } from "@/lib/yogaDosha";

interface DoshaTagGroupProps {
    entries: DoshaEvaluation[];
}

/** Dosha chips in a row with their detail panels expanding below (multi-open, UX spec). */
const DoshaTagGroup = ({ entries }: DoshaTagGroupProps) => {
    const { t } = useI18n();
    ...
    return (
        <div role="group" aria-label={t("yogaDosha.groupDoshaAria")}>
            <h4 className="font-semibold text-sm text-gray-700">
                {t("yogaDosha.doshaGroup")}{" "}
                {present.length > 0 && (
                        ...
                        {present
                            ...
                            .map((entry) => (
                                <YogaDoshaDetailPanel
                                    key={entry.id}
                                    entry={entry}
                                    panelId={`dosha-${entry.id}-panel`}
                                    titleId={`dosha-${entry.id}-title`}
                                />
                            ))}
                    </div>
                </>
