...
describe("calculateHoroscope Yoga/Dosha tags (IT-YD-200..202)", () => {
    // 2012-08-16 12:00 Colombo lahiri: Saturn conjunct Mars in Libra (0.45° orb) — a genuinely
    // PRESENT Shani-Mangala (sm01 conjunction) and Agni-Marutha (am01 + am06 nakshatra overlap).
    const baseData = {
        name: "Test",
        displayName: true,
    ...
    test("IT-YD-200: yogas array, doshas shape and version ride the CalculationResult (persisted shape)", () => {
        const result = calculateHoroscope(baseData);
        expect(result.yogaDoshaVersion).toBe(YOGA_DOSHA_VERSION);
        ...
        expect(shaniMangala).toBeDefined();
        expect(shaniMangala?.isPresent).toBe(true);
        expect(shaniMangala?.formation.rulesTriggered).toEqual(["shaniMangala.sm01"]);

        // Per docs/agni-marutha-dosha.md the broad Saturn–Mars relationship is a separate dosha:
        // a conjunct pair is Agni Marutha too (am01), with its own independent rules.
        const agniMarutha = result.doshas.doshas.find((d) => d.id === "agniMarutha");
        expect(agniMarutha).toBeDefined();
    ...
    test("IT-YD-201: default chart output matches the era-stamped golden fixture (generator round-trip)", () => {
        const result = calculateHoroscope(baseData);
        const snapshot = {
            ascendant: { sign: result.ascendant.sign, degree: Number(result.ascendant.degree.toFixed(2)) },
            planets: result.planets.map((p) => ({ name: p.name, sign: p.sign, house: p.house })),
            yogas: result.yogas.map((y) => ({
                id: y.id,
                isPresent: y.isPresent,
                formation: { primaryRule: y.formation.primaryRule, strength: y.formation.strength },
                finalAssessment: { severity: y.finalAssessment.severity },
                cancellation: { status: y.cancellation.status },
            })),
            doshas: result.doshas.doshas.map((d) => ({ id: d.id, isPresent: d.isPresent })),
        };

        const fixturePath = path.join(process.cwd(), "src/__tests__/fixtures/yoga-dosha-default-2026-09.json");
        if (!fs.existsSync(fixturePath)) {
            // Generator mode: first run writes the fixture (era-stamped) instead of failing.
            fs.writeFileSync(
                fixturePath,
                JSON.stringify(
                    {
                        _comment:
                            "Yoga/Dosha golden fixture — 2012-08-16 12:00 Colombo, lahiri. Auto-generated by IT-YD-201 on the first run; regenerate deliberately (era stamp 2026-09).",
                        ...snapshot,
                    },
                    null,
                    4,
            ...
            console.warn("IT-YD-201: generated a fresh yoga/dosha golden fixture at %s", fixturePath);
            return;
        }

        const fixture = JSON.parse(fs.readFileSync(fixturePath, "utf8")) as typeof snapshot & {
            _comment?: string;
        };
        const { _comment, ...fixtureValues } = fixture;
        expect(snapshot).toEqual(fixtureValues);
    });

    test("IT-YD-202: recalculation job is a zero-change owner of yoga fields (generic spread guard)", () => {
        const src = fs.readFileSync(path.join(process.cwd(), "src/lib/recalculationJob.ts"), "utf8");
        // The auto path stores whatever calculateHoroscope returns — yoga fields ride along in the
        // generic `...calculated` spread, so the job needed NO yoga-specific branching.
        expect(src).toContain("...calculated,");
        expect(src).not.toContain("yogaDosha");
    });
});
